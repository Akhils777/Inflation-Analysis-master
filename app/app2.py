...
def run2():
    ...
    df = load_dataset('Continental_dataset.csv')
    if df.empty:
        return
    ...
    st.write(
        'Compare regional inflation averages over time. '
        'Select regions and date range in the sidebar.'
    )
    ...
    fig = generate_line_chart(df_filtered, 'Regional Inflation Rate Comparison')
    ...
    if not highest.empty:
        st.subheader('Highest Average Regional Inflation')
        st.table(highest)
        st.subheader('Lowest Average Regional Inflation')
        st.table(lowest)

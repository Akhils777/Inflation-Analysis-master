...
def run3():
    ...
    df = load_dataset('War_dataset.csv')
    if df.empty:
        return
    ...
    st.write(
        'Compare inflation during war periods using the available country data. '
        'Use the sidebar to choose countries and date range.'
    )
    ...
    fig = generate_bar_chart(df_filtered, 'War-Time Inflation Rate Comparison')
    ...
    if not highest.empty:
        st.subheader('Highest Average War-Time Inflation by Country')
        st.table(highest)
        st.subheader('Lowest Average War-Time Inflation by Country')
        st.table(lowest)
